...
5. Укажите:
   ...
   - **MQTT Topic:** `msh` (с решеткой - подписка на все сообщения)
...
1. Определяет формат сообщения по топику и настройке `payload_format`
2. Парсит сообщение через `MessageService` (автоматически выбирает JSON или Protobuf парсер)
...
Чтобы не засорять Telegram техническими сообщениями. Пользователям нужны только текстовые сообщения, а техническая информация (`nodeinfo`, `position`) используется для кэша и обновляется автоматически при получении соответствующих пакетов.
...
3. **Правильный топик?**
   - В ноде: `msh/#` (с решеткой - подписка на все сообщения)
...
A: В Telegram отправляются только сообщения типа `text`. Технические сообщения (`nodeinfo`, `position`) обрабатываются для кэша, но не отправляются. Это работает для обоих форматов - JSON и Protobuf.
