...
1. **Компьютер с установленным Docker** (Windows, Mac или Linux)
   - Если Docker не установлен, скачайте с [официального сайта](https://www.docker.com/products/docker-desktop/)
...
# ID сообщения для ответа (опционально, для создания тредов в супергруппах)
# Если указан, все сообщения будут отправляться как ответ на это сообщение
...
**Опционально: Настройка тредов (threads) для супергрупп**

Если вы хотите, чтобы все сообщения от бота были сгруппированы в один тред (thread):

1. Отправьте любое сообщение в группу (или используйте существующее)
2. Получите ID этого сообщения (можно через специальных ботов или API)
3. Добавьте в `.env`:
   ```env
   TELEGRAM_REPLY_TO_MESSAGE_ID=4294970926
   ```
   Где `4294970926` - это ID сообщения, на которое будут отвечать все сообщения от бота
...
Готово! Теперь сообщения от вашей Meshtastic ноды будут приходить в Telegram. Если настроен `REPLY_TO_MESSAGE_ID`, все сообщения будут в одном треде.
...
5. Укажите:
   ...
   - **MQTT Topic:** `msh/2/json/+` (важно: с плюсом, не с решеткой!)
...
│   │   ├── mqtt_handler.py       # Обрабатывает MQTT сообщения
...
- `subscribe(topic, qos)` - подписывается на топик
  - `topic` - на какой топик подписаться (например, `msh/2/json/#`)
...
Превращает сырые MQTT сообщения в структурированные объекты.
...
2. Пытается декодировать как JSON
3. Если это `nodeinfo` - обновляет кэш нод
4. Если это `position` - обновляет координаты в кэше
5. Если это `text` - извлекает текст и информацию об отправителе
6. Создает объект `MeshtasticMessage` со всей информацией
...
- `update_node_info(node_id, longname, shortname)` - обновляет информацию о ноде
  - Сохраняет в память
  - Сохраняет на диск (если прошло 3 дня с последнего обновления)
...
#### `mqtt_handler.py` - Обработка MQTT сообщений

**Класс:** `MQTTMessageHandlerImpl`

Когда приходит сообщение от MQTT брокера, этот класс его обрабатывает.

**Метод:** `handle_message(topic, payload)`

Что делает:

1. Пропускает protobuf сообщения (они дублируют JSON)
2. Парсит сообщение через `MessageService`
3. **Фильтрует:** отправляет в Telegram только сообщения типа `text`
   - `nodeinfo` и `position` обрабатываются для кэша, но не отправляются
4. Форматирует сообщение для Telegram
5. Отправляет в групповой чат
6. Отправляет разрешенным пользователям в личные чаты
7. Пересылает через прокси-сервис

**Почему только `text`?**

Чтобы не засорять Telegram техническими сообщениями. Пользователям нужны только текстовые сообщения, а техническая информация (`nodeinfo`, `position`) используется для кэша.
...
- `topic` - топик для подписки (`msh/2/json/#`)
...
- `reply_to_message_id` - ID сообщения для ответа (опционально, для создания тредов в супергруппах)
...
3. **Правильный топик?**
   - В ноде: `msh/2/json/+` (с плюсом!)
   - В боте: `msh/2/json/#` (с решеткой)
...
**Health checks:**
Docker автоматически проверяет здоровье контейнеров. Если контейнер нездоров - он будет перезапущен.
...
A: Информация о нодах обновляется раз в 3 дня (или принудительно при получении `nodeinfo`). Координаты обновляются в памяти всегда, на диск - раз в 3 дня.

**Q: Почему не все сообщения попадают в Telegram?**  
A: В Telegram отправляются только сообщения типа `text`. Технические сообщения (`nodeinfo`, `position`) обрабатываются для кэша, но не отправляются.
...
2. Реализуйте команды для управления ботом через Telegram
